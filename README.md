...
- EPUB 按 OPF spine item 阅读，支持正文预览、段落选择、标题范围总结、所选段落总结、批注、翻译和集中管理。
- PDF 使用 pdf.js canvas 渲染原版页面，保留图片显示，支持页面阅读、全文搜索和书签。
- 书库支持删除、最近阅读位置恢复、阅读偏好、沉浸阅读模式和章节/页面导航。
- AI 设置支持供应商预设、自定义 Base URL、Model、API Key、prompt 模板和配置错误诊断。
- 总结队列支持暂停、继续、重试和缓存命中；重复点击同一段落/标题优先复用 IndexedDB 中的结果。
- 支持导出当前书 JSON、Markdown 读书笔记、按书籍导出、全库备份导出，以及导入备份。
...
- AI 总结、批注、翻译和 Markdown 笔记整理当前只对 EPUB 阅读开启。
...
模型请求失败时会区分常见配置问题：浏览器连接/CORS、API Key 权限、Base URL 未指向 `/chat/completions`、Model 不被支持、限流或供应商服务错误。
...
- “导出 JSON”会保存当前书的 sections、summaries、comments 等结构化数据，兼容旧版 summary-only 导入。
- “导出 Markdown”会按书内小节顺序整理书签、AI 总结、评论和翻译，生成可直接保存到笔记软件的读书笔记。
- “批注与翻译”面板会集中列出当前 EPUB 的评论和翻译，可跳转到原文片段并删除对应条目。
- 备份导出仍保留原文档 blob、阅读位置、书签、总结、批注和翻译，适合迁移浏览器本地书库。
...
- 已完成：批注/翻译管理面板，可按书籍集中查看、跳转和删除。
- 已完成：导出 Markdown，将总结、评论、翻译和书签整理成读书笔记。
- P1：PDF 扫描件检测和 OCR 提示，避免用户误以为搜索失效。
- P2：PDF AI 总结和批注，复用现有 block/summary cache 语义。
- P2：长 PDF 页面虚拟化，降低大文件内存和 canvas 渲染压力。
- 已完成：AI 配置诊断，提供 CORS、API key、Base URL、model 错误的更明确提示。
